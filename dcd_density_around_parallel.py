...
# -*- coding: utf-8 -*-
"""
Compute a 3D density map of molecule B around molecule A (parallelised version).
...
Output: OpenDX density file (atoms/Å³).
        ...
        Use --nproc to distribute frame processing across multiple worker processes.
        Note: --movie is incompatible with --nproc > 1.
...
def _density_worker(args):
    ...
    ref_fit_np          = args['ref_fit_np']            # (full or subset of native)
    fit_indices_chain   = args['fit_indices_chain']     # None or numpy array
    ...
    for iframe in frame_indices:
        ...
        box_np  = _np.asarray(dcd._header.unit_cell_xyz, dtype=_f64)
        frame_rmsd_list = []

        for sl_a in mol_a_slices:
            mol_a_raw    = coords[sl_a].astype(_f64)
            ...
            rmsd, mat = _calc_rotation(ref_fit_np, mol_a_fit)
            frame_rmsd_list.append(rmsd)

            mol_b_tr_batch = []
            for sl_b in mol_b_slices:
                mol_b_raw        = coords[sl_b].astype(_f64)
                ...
                if _np.any(_np.abs(mol_b_center - mol_a_center) > R + mol_b_global_radius):
                    continue
                mol_b_coords = unwrap_molecule(mol_b_raw + shift, box_np)
                mol_b_tr_batch.append(apply_transform(mat, mol_b_coords))

            if mol_b_tr_batch:
                all_tr   = _np.vstack(mol_b_tr_batch)
                ixyz     = _np.floor((all_tr - origin) / dx).astype(_np.intp)
                mask     = _np.all((ixyz >= 0) & (ixyz < ngrid), axis=1)
                valid    = ixyz[mask]
                if len(valid):
                    flat_idx = (valid[:, 0] * ngrid + valid[:, 1]) * ngrid + valid[:, 2]
                    local_grid += _np.bincount(
                        flat_idx, minlength=ngrid * ngrid * ngrid
                    ).reshape(ngrid, ngrid, ngrid)
            local_samples += 1

        avg_rmsd = _np.mean(frame_rmsd_list) if frame_rmsd_list else float('nan')
        rmsd_list.append((iframe, avg_rmsd))

    dcd.close()
    return local_grid, local_samples, rmsd_list
...
def main():
    ...
    parser.add_argument('--nproc', type=int, default=1,
                        help='Number of parallel worker processes (default: 1). '
                             'Incompatible with --movie.')
    args = parser.parse_args()

    if args.movie and args.nproc > 1:
        print('Warning: --movie is incompatible with --nproc > 1. '
              'Falling back to nproc=1.')
        args.nproc = 1
    ...
    # ----------------------------------------------------------
    # Open DCD — read header only; workers open their own handles
    # ----------------------------------------------------------
    ...
    # ----------------------------------------------------------
    # Movie path: serial loop (unchanged from original)
    # ----------------------------------------------------------
    if args.movie:
        ...
        for iframe in frame_indices:
            ...
            for rank_a, sl_a in enumerate(mol_a_slices):
                ...
                mol_a_tr = apply_transform(mat, mol_a_unwrp)
                ...
                for rank_b, sl_b in enumerate(mol_b_slices):
                    ...
                    mol_b_coords = unwrap_molecule(mol_b_raw + shift, box_np)
                    mol_b_tr = apply_transform(mat, mol_b_coords)
    ...
    # ----------------------------------------------------------
    # Parallel (or serial) density path
    # ----------------------------------------------------------
    else:
        ...
        task_dicts = [{
            ...
            'ref_fit_np'         : ref_fit_np,
            'fit_indices_chain'  : fit_indices_chain,
        ...
        if not task_dicts:
            ...
            with ProcessPoolExecutor(max_workers=args.nproc) as executor:
                futures = [executor.submit(_density_worker, td) for td in task_dicts]
                for fut in futures:
                    local_grid, local_samples, rmsd_list = fut.result()
                    grid          += local_grid
                    total_samples += local_samples
                    all_rmsd.extend(rmsd_list)

        all_rmsd.sort(key=lambda x: x[0])
        for iframe, avg_rmsd in all_rmsd:
            print('Frame {:5d}  avg_RMSD={:.3f} Å'.format(iframe + 1, avg_rmsd))
