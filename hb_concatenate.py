...
def hb_concatenate(dir_in, dir_out, nrun):
   
    f_out = open('%s/azo.hb' % dir_out,'w')
   
    ### Run 1
    f_in = open('%s/run001/azo.hb' % dir_in,'r')
    ...
    for irun in range(2, nrun+1):
        f_in = open('%s/run%03i/azo.hb' % (dir_in,irun,),'r')
...
if __name__ == '__main__':
    ...
    hb_concatenate(dir_in, dir_out, nrun)
