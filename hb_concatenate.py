...
def hb_concatenate(dir_in, name, dir_out, nrun):
   
    f_out = open('%s/%s.hb' % (dir_out,name),'w')
   
    ### Run 1
    f_in = open('%s/run001/%s.hb' % (dir_in,name),'r')
    ...
    for irun in range(2, nrun+1):
        f_in = open('%s/run%03i/%s.hb' % (dir_in,irun,name),'r')
