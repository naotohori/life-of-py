...
import math
import numpy as np
from lop.lib_f2py import py_distance2_hist_nt
...
while dcd.has_more_data() :
    ...
    if len(id0_P) > 0:
        dist2, idx = py_distance2_hist_nt.distance2_hist_nt( data, id0_P, id0_P, r2_hist)
...
for iP in range(nP):
    ...
    if len(id0_Mg) > 0 and len(id0_K) > 0:
        for i in range(nbin):
            ...
            f_out.write('%4.1f %4.1f %7.5f %7.5f %7.5f %7.5f %i %i %i %i\n' % (r1,r2, 
                                         hist_P[iP,i]  / div_factor, 
                                         hist_Mg[iP,i] / div_factor, 
                                         hist_K[iP,i]  / div_factor,
                                         hist_Cl[iP,i] / div_factor,
                                         hist_P[iP,i], hist_Mg[iP,i], hist_K[iP,i], hist_Cl[iP,i]) )
    elif len(id0_K) > 0:
        for i in range(nbin):
            r1 = math.sqrt(bins[i])
            r2 = math.sqrt(bins[i+1])
            ...
            f_out.write('%4.1f %4.1f %7.5f %7.5f %7.5f %7.5f %i %i %i %i\n' % (r1,r2,
                                         ...
                                         hist_K[iP,i] / div_factor, 
                                         hist_Cl[iP,i]/ div_factor,
                                         hist_P[iP,i], 0, hist_K[iP,i], hist_Cl[iP,i]) )
    f_out.write('\n\n')
