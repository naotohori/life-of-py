subroutine count_bound_total( xyz, nmp, id0_P, nP, id0_M, nM, r_bound_max, r_bound_min, boxsize, nbound, ntotal)
       
   implicit none
   integer, intent(in) :: nmp
   integer, intent(in) :: nP
   integer, intent(in) :: nM
   real*8,  intent(in) :: xyz(3, nmp)
   integer, intent(in) :: id0_P(nP)
   integer, intent(in) :: id0_M(nM)
   real*8,  intent(in) :: r_bound_max
   real*8,  intent(in) :: r_bound_min
   real*8,  intent(in) :: boxsize
   integer, intent(out) :: nbound(nP)
   integer, intent(out) :: ntotal
   
   integer :: iP, iM, i
   real*8  :: d2
   real*8  :: xyz_P(3), xyz_M(3), d(3)
   real*8  :: r2_bound_max, r2_bound_min
   real*8  :: half
   logical :: flg_added

   r2_bound_max = r_bound_max ** 2
   r2_bound_min = r_bound_min ** 2
   nbound(:) = 0
   ntotal = 0

   half = 0.5 * boxsize

   do iM = 1, nM
      xyz_M = xyz(:, id0_M(iM)+1)
 
      flg_added = .False.
      !! To prevent double-counting of same ion for the total

      do iP = 1, nP
         xyz_P = xyz(:, id0_P(iP)+1)

         d = xyz_M - xyz_P

         do i = 1, 3
            if (d(i) > half) then
               d(i) = d(i) - boxsize
            else if (d(i) < -half) then
               d(i) = d(i) + boxsize
            endif
         enddo

         d2 = dot_product(d,d)

         if (d2 > r2_bound_min .and. d2 <= r2_bound_max) then
            nbound(iP) = nbound(iP) + 1

            if (.not. flg_added) then
               flg_added = .True.
               ntotal = ntotal + 1
            endif
         endif

      enddo
   enddo

endsubroutine count_bound_total
